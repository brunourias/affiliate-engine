...
def run_pipeline(db:Session,job_id:str,voice=None,scene_renderer=None,composer=None,validator=None):
    ...
    try:
        ...
        precise_duration=sum(x[0].resolved_duration_seconds for x in resolved);pace_status=pace_target_status(precise_duration);job.expected_duration_seconds=ceil(precise_duration);stage(db,job,"COMPOSING",80);output=composer.compose(rendered,job.width,job.height,job.fps);output["durationSeconds"]=precise_duration;log_decision(db,"SYSTEM","MEDIA_JOB","MEDIA_COMPOSING",job.id,metadata={"audioTimelineDuration":precise_duration,"targetDurationSeconds":20,"paceTargetStatus":pace_status});db.commit();stage(db,job,"VALIDATING",95);validation=validator.validate(output);validation.setdefault("details",{}).update({"targetDurationSeconds":20,"actualAudioDurationSeconds":precise_duration,"paceTargetStatus":pace_status,"inputFingerprint":(job.validation_details or {}).get("inputFingerprint")});job.validation_status=validation["status"];job.validation_details=validation.get("details");log_decision(db,"SYSTEM","MEDIA_JOB","MEDIA_VALIDATION_COMPLETED",job.id,metadata={"status":job.validation_status,**(job.validation_details or {})});
