...
class InstagramConnectionService:
    ...
    def public(self)->dict:
        ...
        return {"channel":row.channel,"connector":row.connector,"status":status,"accountDisplayName":row.account_display_name,"accountId":row.account_id,"accountType":row.account_type,"username":row.username,"scopes":row.scopes or [],"expiresAt":row.expires_at,"lastValidatedAt":row.last_validated_at,"tokenHealth":health,"authProfile":row.auth_profile,"appReadiness":ready,"publicationConnectorReadiness":"READY" if status=="CONNECTED" and not missing and row.account_type in {"BUSINESS","CREATOR"} else "NOT_READY"}
    ...
    def validate(self)->dict:
        ...
        account_type=str(account.get("account_type") or account.get("accountType") or row.account_type or "").upper();row.account_type=account_type;row.username=account.get("username") or row.username;row.account_display_name=row.username;row.last_validated_at=utcnow();missing=ScopeValidator().missing(row.scopes or []);row.status="ACCOUNT_NOT_SUPPORTED" if account_type not in {"BUSINESS","CREATOR"} else "INSUFFICIENT_SCOPE" if missing else "CONNECTED";log_decision(self.db,"OPERATOR","PUBLICATION_CONNECTION","INSTAGRAM_CONNECTION_VALIDATED",row.id,metadata={"status":row.status,"accountType":row.account_type,"scopes":row.scopes});self.db.commit();return self.public()
