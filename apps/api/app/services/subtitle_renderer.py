...
class SubtitleRenderer:
    ...
    def write_headline(self,text,duration,directory:Path,stem:str,width:int,height:int,layout="TOP_BANNER"):
        directory.mkdir(parents=True,exist_ok=True);path=directory/f"{stem}_headline.ass";size=max(28,round(height*.044));alignment={"LEFT_BLOCK":4,"RIGHT_BLOCK":6,"BOTTOM_SAFE":2}.get(layout,8);margin_x=round(width*.08);margin_v=round(height*(.14 if alignment==8 else .25));safe="\\N".join(blocks(text.upper(),24)[:2]);header=f"[Script Info]\nScriptType: v4.00+\nPlayResX: {width}\nPlayResY: {height}\n\n[V4+ Styles]\nFormat: Name, Fontname, Fontsize, PrimaryColour, OutlineColour, Bold, Alignment, MarginL, MarginR, MarginV, Outline, Shadow\nStyle: Headline,{settings.media_font_name},{size},&H00FFFFFF,&H00102020,-1,{alignment},{margin_x},{margin_x},{margin_v},3,1\n\n[Events]\nFormat: Layer, Start, End, Style, Text\n"
        path.write_text(header+f"Dialogue: 1,0:00:00.00,{timestamp(duration,True)},Headline,{safe}\n",encoding="utf-8");return path
