...
const phases = [['V1-A', 'Foundation + Control Center', 'HOMOLOGADA'], ['V1-B.1', 'Conexão + diagnóstico de capacidades', 'HOMOLOGADA'], ['V1-B.2', 'Mercado Livre Radar', 'HOMOLOGADA'], ['V1-C.1', 'Curator Intake + Evidence', 'ATUAL · EM HOMOLOGAÇÃO'], ['V1-C.2', 'Curator Decision Engine', 'PRÓXIMA'], ['V1-D', 'Campaign Engine', 'PLANEJADO'], ['V1-E', 'Creative Studio', 'PLANEJADO'], ['V1-F', 'Video Pipeline', 'PLANEJADO'], ['V1-G', 'Social Publishers', 'PLANEJADO'], ['V1-H', 'Analytics', 'PLANEJADO'], ['V1-I', 'Learning Engine + Agent Chat', 'PLANEJADO'], ['V1-J', 'External Intelligence', 'PLANEJADO']];

export function Roadmap() {
  return <><header className="page-head"><div><span>EVOLUÇÃO DO PRODUTO</span><h1>Roadmap</h1><p>O que existe agora e o que permanece deliberadamente fora do banco e da interface.</p></div></header><Section title="Fases"><div className="timeline">{phases.map(([id, title, status]) => <div className={id === 'V1-C.1' ? 'current' : ''} key={id}><i>{id}</i><span><b>{title}</b><small>{status}</small></span></div>)}</div></Section><Section title="Módulos futuros"><div className="future-grid">{['Produtos enriquecidos', 'Preços', 'Campanhas', 'Criativos', 'Publicações', 'Analytics', 'Inteligência', 'Audiência própria'].map(item => <div key={item}><LockKeyhole /><b>{item}</b><span>Não implementado na V1-C.1</span></div>)}</div></Section></>;
}
