...
const phases = [['V1-A', 'Foundation + Control Center', 'HOMOLOGADA'], ['V1-B.1', 'Conexão + diagnóstico de capacidades', 'HOMOLOGADA'], ['V1-B.2', 'Mercado Livre Radar', 'HOMOLOGADA'], ['V1-C.1', 'Curator Intake + Evidence', 'HOMOLOGADA'], ['V1-C.2', 'Curator Decision Engine', 'HOMOLOGADA'], ['V1-D', 'Campaign Engine', 'HOMOLOGADA'], ['V1-E', 'Creative Studio', 'HOMOLOGADA'], ['V1-F', 'Local Media Engine', 'PRÓXIMA'], ['V1-G', 'Social Publishers', 'PLANEJADO'], ['V1-H', 'Analytics / Learning Loop', 'PLANEJADO']];

export function Roadmap() {
  return <><header className="page-head"><div><span>EVOLUÇÃO DO PRODUTO</span><h1>Roadmap</h1><p>O que existe agora e o que permanece deliberadamente fora do banco e da interface.</p></div></header><Section title="Fases"><div className="timeline">{phases.map(([id, title, status]) => <div className={id === 'V1-F' ? 'current' : ''} key={id}><i>{id}</i><span><b>{title}</b><small>{status}</small></span></div>)}</div></Section><Section title="Módulos futuros"><div className="future-grid">{['Brand Assets', 'Avatar Motion', 'Voz local', 'Legendas', 'Imagem e posts', 'Composição de vídeo', 'FFmpeg', 'Publicações'].map(item => <div key={item}><LockKeyhole /><b>{item}</b><span>Não implementado na V1-E</span></div>)}</div></Section></>;
}
