...
export function CreativeMediaPage(){const{id=''}=useParams();const load=useCallback(async()=>{const creative=await creativesApi.get(id);if(creative.status!=='APPROVED')return null;const[campaign,scenes]=await Promise.all([campaignsApi.get(creative.campaignId),creativesApi.scenes(id)]);return{creative,campaign,requiresVoice:scenes.some(scene=>Boolean(scene.narrationText)&&scene.speaker!=='NONE')}},[id]);const{data}=useLoad(load);if(!data)return null;return <><FormatDecision creativeId={data.creative.id}/><ChannelVariants creativeId={data.creative.id}/><PublicationReadinessPanel creativeId={data.creative.id}/><PublicationConnectors creativeId={data.creative.id} disclosureText={data.creative.disclosureText}/><StaticProduction creativeId={data.creative.id}/><MediaProduction creativeId={data.creative.id} candidateId={data.campaign.candidateId} requiresVoice={data.requiresVoice}/></>}
